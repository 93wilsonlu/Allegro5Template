...
#include "utility.h"
// <Forward Declaration>
// forward declarations can be useful when you need to have looping struct
// declarations, but you want to (should) prevent mutually include dependency.
// Only applicable when you only need the declaration but no need the definition
// in you sturct, which means that you only used the struct as pointer(no access
// to struct data), param, return type in this file. If you have to access the
// data in struct ex. `A->data`, you should include the corresponding header
// file for its definition. Advance reading :
...
bool ghost_movable(const Ghost* ghost,
                   ...
                   bool room);
/* check if the direction is ok to move_script in; if room = true, then room
 * will be treat as not able to move_script in */
