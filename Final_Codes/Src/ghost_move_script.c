...
static void ghost_move_script_FLEE(Ghost* ghost,
                                   Map* M,
...
static void ghost_move_script_FREEDOM_random(Ghost* ghost, Map* M) {
    ...
    */
    ...
    // But your random strategy have to designed carefully so that ghost won't
    // walk back and forth. (The code above DO perform walking back and forth.)
...
static void ghost_move_script_FREEDOM_shortest_path(Ghost* ghost,
                                                    Map* M,
                                                    Pacman* pman) {
    ghost->objData.nextTryMove =
        shortest_path_direc(M, ghost->objData.Coord.x, ghost->objData.Coord.y,
...
static void ghost_move_script_GO_IN(Ghost* ghost, Map* M) {
    // Description
    // `shortest_path_direc` is a function that returns the direction of
    // shortest path. Check `map.c` for its detail usage. For GO_IN state.
    ghost->objData.nextTryMove =
        shortest_path_direc(M, ghost->objData.Coord.x, ghost->objData.Coord.y,
                            cage_grid_x, cage_grid_y);
}
...
static void ghost_move_script_GO_OUT(Ghost* ghost, Map* M) {
    ...
    // You should modify here if you have different implementation/design of
    // room.
...
static void ghost_move_script_FLEE(Ghost* ghost,
    ...
    // Description:
    // The concept here is to simulate ghosts running away from pacman while
    // pacman is having power bean ability. To achieve this, think in this way.
...
void ghost_move_script_shortest_path(Ghost* ghost, Map* M, Pacman* pacman) {
    ...
    // Since always shortest path strategy is too strong, player have no chance
    ...
    switch (ghost->status) {
        case BLOCKED:
            ghost_move_script_BLOCKED(ghost, M);
            if (al_get_timer_count(game_tick_timer) - ghost->go_in_time >
                GO_OUT_TIME)
                ghost->status = GO_OUT;
            break;
        case FREEDOM:
