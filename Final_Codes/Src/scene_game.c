...
static void init(void) {
    ...
    */
    if (!ghosts) {
        ...
        // Try to look the definition of ghost_create in ghost.h and figure out
...
static void checkItem(void) {
    ...
    if (Grid_y >= basic_map->row_num - 1 || Grid_y <= 0 ||
        ...
        return;
    // TODO-HACKATHON 1-3: check which item you are going to eat and use
    // `pacman_eatItem` to deal with it.
    /*
...
static void status_update(void) {
    ...
    if (pman->powerUp)
    {
            ...
            // If runs out of time reset all relevant variables and ghost's
    status
...
static void on_key_down(int key_code) {
    switch (key_code) {
            // TODO-HACKATHON 1-1: Use allegro pre-defined enum
            // ALLEGRO_KEY_<KEYNAME> to controll pacman movement we provided you
            // a function `pacman_NextMove` to set the pacman's next move
            // direction.
            /*
            case ALLEGRO_KEY_W:
