...
ALLEGRO_BITMAP* load_bitmap_resized(const char* filename, int w, int h) {
    ...
    if (!resized_bmp)
        game_abort("failed to create bitmap when creating resized image: %s",
                   filename);
...
bool pnt_in_rect(int px, int py, RecArea field) {
    return field.x <= px && px <= field.x + field.w && field.y <= py &&
           py <= field.y + field.h;
}
...
bool RecAreaOverlap(const RecArea* const RA, const RecArea* const RB) {
    ...
    if ((fmin(RA_x2, RB_x2) > fmax(RA->x, RB->x)) &&
        (fmin(RA_y2, RB_y2) > fmax(RA->y, RB->y)))
        return true;
    return false;
}
RecArea getDrawArea(object* obj, uint32_t TOTAL_TICK) {
    // NOTODO: return the drawing RecArea defined by object and GAME_TICK_CD
    // To understand why getDrawArea((object*)ghost, ...) works, please read
...
bool bernoulliTrail(double p) {
    ...
    if (p >= 1 || p <= 0)
        game_abort(
            "Error range of p = %lf in BernoulliTrail func\n But p should be "
            "between 0.0 and 1.0",
            p);
