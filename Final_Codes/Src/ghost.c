...
void ghost_draw(Ghost* ghost) {
    ...
    // comments in pacman_draw Since ghost has more status, we suggest you
    // finish pacman_draw first. The logic is very similar.
...
void ghost_toggle_FLEE(Ghost* ghost, bool setFLEE) {
    ...
    // When pacman eats the power bean, only ghosts who are in state FREEDOM
