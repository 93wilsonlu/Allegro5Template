...
// For `Button` struct(object) is defined in `scene_menu_object.h` and
// `scene_menu_object.c` STRONGLY recommend you trace both of them first.
...
static void draw() {
    ...
    // draw title image
    al_draw_scaled_bitmap(gameTitle, 0, 0, gameTitleW, gameTitleH, offset_w,
                          offset_h, gameTitleW * scale, gameTitleH * scale, 0);
    al_draw_text(menuFont, al_map_rgb(255, 255, 255), SCREEN_W / 2,
                 SCREEN_H - 150, ALLEGRO_ALIGN_CENTER, "PRESS <ENTER>");
...
static void on_mouse_move(int a, int mouse_x, int mouse_y, int f) {
    //	TODO-HACKATHON 3-7: Update button's status(hovered), and utilize the
    //function `pnt_in_rect`, which you just implemented 	Uncomment and fill the
    //code below 	 btnSettings.hovered = ???(btnSettings, mouse_x, mouse_y);
}
