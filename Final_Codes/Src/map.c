...
const int map_offset_x = 25,
          map_offset_y = 50;  // pixel offset of where to start draw map
...
Map* create_map(const char* filepath) {
    ...
    if (filepath == NULL) {
        M->row_num = 30;
        M->col_num = 36;
        game_log("Creating from default map. row = %d col = %d", M->row_num,
                 M->col_num);
    ...
    /*
            Allocate a 2-Dimension dynamic char array for recording Map at
       M->map
    */
    ...
    M->wallnum = M->beansCount =
        0;  // * Record the number of beans and walls, which can be used to
            // print score or other usage.
    for (int i = 0; i < M->row_num; i++) {
...
static void draw_block_index(const Map* M, const int row, const int col) {
    ...
    if (!(U && UR && UL && B && BR && BL && R && L)) {
        ...
        if (row < M->row_num - 1 && B && !(BL && BR && R && L)) {
            ...
            al_draw_filled_rectangle(s_x, s_y, e_x, e_y,
...
static void draw_bean(const Map* M, const int row, const int col) {
    al_draw_filled_circle(
        map_offset_x + col * block_width + block_width / 2.0,
...
bool is_wall_block(const Map* M, int index_x, int index_y) {
    if (index_x < 0 || index_x >= M->col_num || index_y < 0 ||
        index_y >= M->row_num)
        return true;
...
Directions shortest_path_direc(Map* M,
                               int startGridx,
                               int startGridy,
                               int endGridx,
                               int endGridy) {
    // NOTODO
    // Here is a complete function return the next direction of the shortest
    // path. Given Map, start point and end point. It will tell you where to go
    // for the shortest path. !NOTICE! if your map grow really large, the size
    // of queue, may become not enough. Hint: You can alter this function and
    // make it return direction and also the distance for your usage.
    ...
    steped[startGridy][startGridx] =
        1; /*	for dummy just means that startGrid have been visited.	*/
