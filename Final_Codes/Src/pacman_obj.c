...
static bool pacman_movable(const Pacman* pacman,
    ...
    // `is_wall_block` and `is_room_block` to check if the block is wall or
    // room. (they are both defined in map.c) 2) the coordinate data of pacman
    // is stored in pacman->objData.Coord it is a self-defined pair IntInt type.
    // Trace the code and utilize it.
...
void pacman_draw(Pacman* pman) {
    ...
    al_draw_scaled_bitmap(
        pman->move_sprite, 0, 0, 16, 16, drawArea.x + fix_draw_pixel_offset_x,
    ...
    if (!game_over) {
        ...
        // animation to draw. For example, if the value if odd, draw 1st frame.
        // Otherwise, draw 2nd frame. But this frame rate may be a little bit
        // too high. We can use % 32 and draw 1st frame if value is 0~15, and
        // 2nd frame if value is 16~31.
        /*
        ...
        */
        ...
        case LEFT:
                al_draw_scaled_bitmap(pman->move_sprite, ... + offset, 0,
                        16, 16,
                        drawArea.x + fix_draw_pixel_offset_x, drawArea.y +
        fix_draw_pixel_offset_y, draw_region, draw_region, 0
                );
                break;
        case RIGHT:
                al_draw_scaled_bitmap(pman->move_sprite, ... + offset, 0,
