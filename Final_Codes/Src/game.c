...
static void allegro5_init(void) {
    ...
    // Register display, timer, tick timer, keyboard, mouse events to the event
    // queue.
    al_register_event_source(game_event_queue,
                             al_get_display_event_source(game_display));
    al_register_event_source(game_event_queue,
                             al_get_timer_event_source(game_tick_timer));
    al_register_event_source(game_event_queue,
                             al_get_timer_event_source(game_update_timer));
    ...
    while (!gameDone) {
        ...
        if (event.type == ALLEGRO_EVENT_DISPLAY_CLOSE) {
            ...
            if (event.keyboard.keycode == ALLEGRO_KEY_ESCAPE &&
                strcmp(active_scene.name, "Menu") == 0) {
            ...
            // game_log("Mouse button %d down at (%d, %d)", event.mouse.button,
            // event.mouse.x, event.mouse.y);
            mouse_state[event.mouse.button] = true;
            if (active_scene.on_mouse_down)
                (*active_scene.on_mouse_down)(event.mouse.button, event.mouse.x,
                                              event.mouse.y, 0);
        } else if (event.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP) {
            // Event for mouse key up.
            // game_log("Mouse button %d up at (%d, %d)", event.mouse.button,
            // event.mouse.x, event.mouse.y);
            mouse_state[event.mouse.button] = false;
            if (active_scene.on_mouse_up)
                (*active_scene.on_mouse_up)(event.mouse.button, event.mouse.x,
                                            event.mouse.y, 0);
        } else if (event.type == ALLEGRO_EVENT_MOUSE_AXES) {
            if (event.mouse.dx != 0 || event.mouse.dy != 0) {
                // Event for mouse move_script.
                // game_log("Mouse move_script to (%d, %d)", event.mouse.x,
                // event.mouse.y);
                mouse_x = event.mouse.x;
                mouse_y = event.mouse.y;
                if (active_scene.on_mouse_move)
                    (*active_scene.on_mouse_move)(0, event.mouse.x,
                                                  event.mouse.y, 0);
            } else if (event.mouse.dz != 0) {
                // Event for mouse scroll.
                // game_log("Mouse scroll at (%d, %d) with delta %d",
                // event.mouse.x, event.mouse.y, event.mouse.dz);
                if (active_scene.on_mouse_scroll)
                    (*active_scene.on_mouse_scroll)(
                        0, event.mouse.x, event.mouse.y, event.mouse.dz);
            }
        }
